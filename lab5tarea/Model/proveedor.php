...
class proveedor{
    ...
    public function Obtener($nit){
        try{
            ...
            return $stm->fetch(PDO::FECTH_OBJ);
        }catch(Exception $e){
            die($e->getMessage());
        }
    }
    ...
    public function Actualizar($data){
        try{
            $sql="UPTDATE proveedor SET razonS=?, dir=?, tel=? WHERE nit=?";
            $this->pdo->prepare($sql)->execute(array($data->razonS,$data->dir,$data->tel,$data->nit));
        }catch(Exception $e){
            die($e->getMessage());
        }
    }
    public function Registrar(proveedor $data){
        try{
            ...
            $this->pdo->prepare($sql)->execute(array($data->nit,$data->razonS,$data->dir,$data->tel));
        }catch(Exception $e){
            die($e->getMessage());
        }  
    }
